...
// --- [FIX START] ---
// Read data from POST (x-www-form-urlencoded) first
$data = $_POST;

// If $_POST is empty, check for raw JSON input
if (empty($data)) {
...
// Now check the $data array instead of $_GET
...
try {
    // 1. Fetch main place details from the 'places' table (Unchanged)
    ...
    // 2. Fetch all related images, including their unique IDs (Unchanged)
    ...
    // 3. Fetch all related top spots, including their unique IDs (Unchanged)
    ...
    // 4. Fetch all related transport options, including their unique IDs (Unchanged)
    ...
    // 5. Calculate average rating and review count from the 'reviews' table (Unchanged)
    ...
    // 6. Ensure numeric fields from the database are correctly typed as numbers in the JSON (Unchanged)
    ...
    // 7. Assemble the final successful response (Unchanged)
