...
function manualVerifyTokenSignature($id_token) { /* ... keep the function code from the previous version ... */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($payload) {
            ...
            $stmt->close();
            if ($user) { $user_id = $user['id']; $fullname = $user['fullname']; $email = $user['email'];
            } else {
                ...
                $random_password = password_hash(bin2hex(random_bytes(16)), PASSWORD_DEFAULT); $insertStmt = $conn->prepare("INSERT INTO users (fullname, email, password, username) VALUES (?, ?, ?, ?)"); $insertStmt->bind_param("ssss", $fullname, $email, $random_password, $username); $insertStmt->execute(); $user_id = $insertStmt->insert_id; $insertStmt->close();
            }
            $session_token = bin2hex(random_bytes(32)); $updateStmt = $conn->prepare("UPDATE users SET session_token = ? WHERE id = ?"); $updateStmt->bind_param("si", $session_token, $user_id); $updateStmt->execute(); $updateStmt->close();
